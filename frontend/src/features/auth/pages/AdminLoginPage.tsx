  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (session.user.role !== "ADMIN" && session.user.role !== "STAFF") {
        authStore.clear();
        setError("This portal is for administrators and staff only.");
        return;
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#2C1810] to-[#4A2C1A] flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-md bg-[#FBF8F3] rounded-2xl border-2 border-[#D4A574]/40 shadow-2xl p-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-xl text-[#2C1810] font-semibold">Admin sign-in</h1>
            <p className="text-xs text-[#6B5D52]">Kappio Café operations</p>
          </div>
        </div>

        <p className="text-sm text-[#6B5D52] mb-6 mt-4">
          Internal use only. For customer ordering, use the public site.
        </p>
